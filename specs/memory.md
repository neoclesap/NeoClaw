# Spec: Memory Module (SQLite Persistence) — NeoClaw
...
O módulo de persistência de estado do **NeoClaw** gerencia tanto as conversas de longo prazo em banco de dados SQLite (`better-sqlite3`) quanto atua como manager da janela de contexto para impedir que o limite máximo do envelope de tokens da IA (Context Window) estoure.
