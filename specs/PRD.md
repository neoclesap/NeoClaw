...
O **NeoClaw** é um agente pessoal de Inteligência Artificial para operar 100% localmente no desktop do usuário. Ele recebe comandos exclusivamente pelo Telegram, processa-os através de um pipeline que suporta múltiplos LLMs dinamicamente e tem acesso persistente à memória em SQLite.
...
- NG-03: Não terá suporte a bancos de dados robustos como PostgreSQL/Mongo. Foco exclusivo em SQLite local mínimo.

---

## 5. Usuários e Personas

**Usuário primário:** neocles (proprietário), acessando via dispositivo móvel ou desktop via cliente Telegram, utilizando IDs em whitelist restrita.
...
O usuário envia um chat no Telegram, o NeoClaw roda local em background num terminal, chama LLMs, lê Skills em pastas locais, aciona ferramentas e responde no mesmo chat de forma orgânica.
...
| RF-02 | O sistema deve validar todas as mensagens entrantes contra a variável `TELEGRAM_ALLOWED_USER_IDS` | Must | Um usuário não cadastrado recebe ignore instantâneo; nenhum log sensível é disparado e nenhuma API key é exposta. |  
...
|----|-----------|------------|------------|  
| RNF-01 | Latência de repassagem de Msg | < 1000ms | Não confundir atraso do bot com o da API do provedor LLM. |  
...
| EC-03: Key Inválida | O arquivo `.env` está corrompido ou API key descontinuada | Agent tenta iniciar, loga Erro fatal de auth no Terminal e notifica no log que o provider `X` falhou. |
