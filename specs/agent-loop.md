# Spec: Agent Loop (Reasoning Engine) — NeoClaw
...
O **Agent Loop** é a engrenagem central do **NeoClaw**. Ele implementa o padrão ReAct (Reasoning and Acting). É o módulo onde uma ação bruta entra, é submetida ao LLM base (Thought), uma ou mais ferramentas são chamadas (Action + Observation), até que um veredito de resposta final seja alcançado, repetindo em loop limitado de iterações para evitar impasses de contexto infinito.
